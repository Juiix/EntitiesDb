...
internal static class Discovery
{
	internal static bool IsCandidateNode(SyntaxNode node, System.Threading.CancellationToken _)
	{
		...
		{
			"ForEach" or "ForEachParallel" or "ForEachChunk" or "ForEachChunkParallel" => true,
			_ => false
		};
	...
	internal static InvocationCapture Transform(GeneratorSyntaxContext ctx, System.Threading.CancellationToken _)
	{
		...
		{
			"ForEach" => MethodType.ForEach,
			"ForEachParallel" => MethodType.ForEachParallel,
		...
		IOperation lambdaOp;
		IOperation methodGroupOp;
		if (!TryGetCallable(sm, actExpr, out lambdaOp, out methodGroupOp))
			return InvocationCapture.Invalid;

		var anon = lambdaOp as IAnonymousFunctionOperation;
		if (anon == null) return InvocationCapture.Invalid;

		// Optional: forbid captured state (safe static generation)
		if (LambdaCaptures(anon)) return InvocationCapture.Invalid;

		var lamSym = anon.Symbol;
	...
	private static bool TryGetCallable(SemanticModel sm, SyntaxNode expr, out IOperation lambda, out IOperation methodGroup)
	{
		lambda = null; methodGroup = null;
		var op = sm.GetOperation(expr);

		var anon = op as IAnonymousFunctionOperation;
		if (anon != null) { lambda = anon; return true; }

		var del = op as IDelegateCreationOperation;
		if (del != null)
		{
			var tgt = del.Target as IAnonymousFunctionOperation;
			if (tgt != null) { lambda = tgt; return true; }
		}

		var conv = op as IConversionOperation;
		if (conv != null)
		{
			var innerAnon = conv.Operand as IAnonymousFunctionOperation;
			if (innerAnon != null) { lambda = innerAnon; return true; }

			var innerMg = conv.Operand as IMethodReferenceOperation;
			if (innerMg != null) { methodGroup = innerMg; return true; }
		}

		var mg = op as IMethodReferenceOperation;
		if (mg != null) { methodGroup = mg; return true; }
