...
internal static partial class ModelBuilder
{
	...
	private static ForEachRenderModel BuildModel(InvocationCapture cap)
	{
		...
		if (hasEntity)
		{
			handleLines.Add("var entityHandle = chunk.GetEntityHandle();");
		...
		for (int i = 0; i < comps.Count; i++)
		{
			...
			switch (c.Kind)
			{
				...
				case ParamKind.ComponentHandle:
					h = "var " + baseVar + " = chunk.GetHandle(" + offsetsName + "." + ofs + ");"; break;
				case ParamKind.ComponentIn:
				case ParamKind.ComponentReadOnlyHandle:
					h = "var " + baseVar + " = chunk.GetHandleReadOnly(" + offsetsName + "." + ofs + ");"; break;
				case ParamKind.Buffer:
				case ParamKind.BufferHandle:
					h = "var " + baseVar + " = chunk.GetBufferHandle(" + offsetsName + "." + ofs + ");"; break;
				case ParamKind.ReadOnlyBuffer:
				case ParamKind.ReadOnlyBufferHandle:
					h = "var " + baseVar + " = chunk.GetBufferHandleReadOnly(" + offsetsName + "." + ofs + ");"; break;
				default:
					h = ""; break;
			}
