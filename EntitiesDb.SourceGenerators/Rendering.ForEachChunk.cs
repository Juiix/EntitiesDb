...
{
	private static string RenderForEachChunk(ForEachRenderModel m)
	{
		...
		// Method header
		w.Append("public static void ForEachChunk")
		...
		// main loops
		w.AppendLine("foreach (var archetype in self.GetArchetypeIterator())");
		w.AppendLine("{");
		w.Indent();
