...
public sealed class EntityDatabaseTests
{
	...
	public void DestroyEntity_Removes_And_Compacts()
	{
		...
		Assert.Equal(77, db.Get<Health>(b.Id).Value);
	...
	public void CloneEntity_CopiesAllComponents()
	{
		...
		var p = db.Get<Position>(clone.Id);
		var n = db.Get<NameTag>(clone.Id);
		Assert.Equal(10, p.X);
		Assert.Equal(-5, p.Y);
		Assert.Equal("src", n.Text);

		// Buffered copied (content visible)
		var cloneBuf = db.GetBuffer<Damage>(clone.Id);
	...
	public void Add_Get_UnmanagedComponent_Works_ByRef()
	{
		...
		same.X = 9;
		Assert.Equal(9, db.Get<Position>(e.Id).X);
	}
	...
	public void Add_Get_ManagedComponent_Works()
	{
		...
		tag2.Text = "beta";
		Assert.Equal("beta", db.Get<NameTag>(e.Id).Text);
	}
	...
	public void GetComponent_Throws_WhenMissing()
	{
		var db = CreateDb();
		var e = db.Create();
		var ex = Assert.Throws<ComponentException>(() => db.Get<Position>(e.Id));
	...
	public void AddBuffer_GetBuffer_RemoveBuffer_HappyPath()
	{
		...
		var view = db.GetBuffer<Damage>(e.Id);
	...
	public void GetBuffer_OnNonBufferedType_Throws()
	{
		...
		var e = db.Create();
		db.Add<Position>(e.Id, new Position { X = 1, Y = 2 });
		Assert.Throws<ComponentException>(() => db.GetBuffer<Position>(e.Id));
	}
